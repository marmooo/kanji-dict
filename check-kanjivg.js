...
const jis1 = Deno.readTextFileSync("jis/1.lst").trimEnd().split("\n");
const jis2 = Deno.readTextFileSync("jis/2.lst").trimEnd().split("\n");
const jis3 = Deno.readTextFileSync("jis/3.lst").trimEnd().split("\n");
const jis4 = Deno.readTextFileSync("jis/4.lst").trimEnd().split("\n");
...
function checkStatus(kanjiList, arr, name) {
  let included = true;
  for (let i = 0; i < arr.length; i++) {
    if (!kanjiList.includes(arr[i])) {
      included = false;
      break;
    }
  }
...
for (const file of expandGlobSync("kanjivg/*.svg")) {
  ...
  const basename = arr[arr.length - 1].split(".")[0];
  const kanjiId = basename.split("-")[0];
  const kanji = toKanji(kanjiId);
  kanjiList.push(kanji);
}

checkStatus(kanjiList, s1, "小1");
checkStatus(kanjiList, s2, "小2");
checkStatus(kanjiList, s3, "小3");
checkStatus(kanjiList, s4, "小4");
checkStatus(kanjiList, s5, "小5");
checkStatus(kanjiList, s6, "小6");
checkStatus(kanjiList, j2, "中2");
checkStatus(kanjiList, j3, "中3");
checkStatus(kanjiList, a1, "常用");
checkStatus(kanjiList, jis1, "JIS1");
checkStatus(kanjiList, jis2, "JIS2");
checkStatus(kanjiList, jis3, "JIS3");
checkStatus(kanjiList, jis4, "JIS4");
