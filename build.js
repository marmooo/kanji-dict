...
const dirNames = [null, '小1', '小2', '小3', '小4', '小5', '小6', '中2', '中3', '常用', '常用外'];
const grades = [null, '小学1年生', '小学2年生', '小学3年生', '小学4年生', '小学5年生', '小学6年生', '中学1〜2年生', '中学3年生', '常用漢字', '常用外漢字'];
...
function generateNonRegularKanjiList() {
  var list = {};
  for (let i=1; i<=4; i++) {
    const kanji = fs.readFileSync(`jis/${i}.lst`).toString().split("\n");
    list[kanji] = true;
  }
...
function getBushuInfo() {
  ...
  readEachLineSync('bushu-list-20110425.csv', 'utf8', (line) => {
    var [kanji, _partsName, names] = line.split(',');
...
function getProperIdioms(words, kanji, level) {
  const examples = [];
  words.forEach(word => {
...
function getBuildInfo() {
  ...
  for (var [kanji, info] of Object.entries(kanjiInfo)) {
    ...
    var level = getGrade(kanji);
    if (level <= 0) {
      console.log('error1: ' + kanji);
      continue;
    }
    var vocab = getGradedVocab(kanji, level).filter(word => !/^[一-龠々]+$/.test(word));
    var idioms = getGradedIdioms(kanji, level);
    var examples = getProperIdioms(idioms.concat(vocab), kanji, level);
...
for (var level=1; level<gradeByKanjis.length; level++) {
  var dir = 'src/' + dirNames[level];
